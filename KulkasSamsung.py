...
for listKulkas in listKulkases:
    tipeKulkas = listKulkas.find(
        'span', class_='cm-shop-card__serial').get_text()
    namaKulkas = listKulkas.find('strong', class_='s-txt-title').get_text()
    try:
        hargaKulkas = listKulkas.find(
            'span', class_='cm-shop-card__price-num').get_text()
    except:
        hargaKulkas = 0
    dataSamsung.append((tipeKulkas, namaKulkas, hargaKulkas))

df = pd.DataFrame(dataSamsung, columns=[
                  'tipeKulkas', 'namaKulkas', 'hargaKulkas'])
